...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- NEW: Hide Section 1 immediately when intro starts to avoid any flash ---
  function hideSection1Now() {
    hero1.style.opacity = '0';
    hero1.style.height  = '0';
    // Hide instantly (no waiting until intro ends)
  ...
  function showIntroVideo() {
    ...
    // Show intro layer on top of a black stage
    ...
    introVideo.onended = () => openSection2Direct();

    // Safety: if video fails, proceed after 30s
    setTimeout(() => {
  ...
  function openSection2Direct() {
    ...
    // Reveal Page 2 main — THIS IS THE ONLY SCROLLER
    page2Main.classList.remove('hidden');

    // Reveal first card & scroll into view
    const first = document.getElementById('khmer-invite');
...
/* Expose closeModal for inline onclick fallback */
